...
/*-------------------------------------------------
 * Touchscreen, mouse/touchpad or keyboard as libinput interface (for Linux
 *based systems)
 *------------------------------------------------*/
#ifndef USE_LIBINPUT
...
/*If only a single device of the same type is connected, you can also auto
 *detect it, e.g.: #define LIBINPUT_NAME
 *libinput_find_dev(LIBINPUT_CAPABILITY_TOUCH, false)*/
...
#define XKB_KEY_MAP                                                            \
  {                                                                            \
    .rules = NULL, .model = "pc101", .layout = "us", .variant = NULL,          \
    .options = NULL                                                            \
