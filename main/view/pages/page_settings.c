...
static void show_msgbox(model_t *pmodel, struct page_data *pdata, lv_obj_t *msgbox);
...
static void create_menu_btn(lv_pman_handle_t handle, int btn_id, char *text, int y) {
    lv_obj_t *label;
...
static lv_obj_t *my_msgbox_create(lv_pman_handle_t handle, struct msgbox_t msgbox) {
    const uint32_t MSGW = 600, MSGH = 300;

    LV_LOG_INFO("begin");
    if (msgbox.obj == NULL) {
        *msgbox.obj = lv_obj_class_create_obj(&lv_msgbox_backdrop_class, lv_layer_top());
...
static void open_page(lv_pman_handle_t handle, void *args, void *state) {
    ...
    int   btn_n       = 6;
    int   btn_ids[]   = {BTN_NEW_VERSION, BTN_EXPORT, BTN_START_APP, BTN_SUP_VERSION, BTN_APP_VERSION, BTN_CLOSE};
    char *btn_texts[] = {"New version",        "Export diagnose files", "Start the application",
                         "Supervisor version", "Application version",   "Close"};
    ...
    static int      msgbox_start_already_btn_ids[]   = {BTN_START_APP_RESTART, BTN_START_APP_CANCEL};
    static char    *msgbox_start_already_btn_texts[] = {"RESTART", "CANCEL", "\0"};
    struct msgbox_t m_app_start_already              = {.obj       = &pdata->msgbox_app_start_already,
                                                        .title     = "Application",
    ...
    struct msgbox_t m_app_start_succ = {.obj       = &pdata->msgbox_app_start_succ,
                                        ...
                                        .btn_ids   = NULL,
                                        .btn_texts = NULL};
    struct msgbox_t m_app_start_err  = {.obj   = &pdata->msgbox_app_start_err,
                                        .title = "Application status",
                                        .text  = "Failed to start application\nPlease check that the path is correct",
                                        .btn_close = true,
                                        .btn_ids   = NULL,
                                        .btn_texts = NULL};
    struct msgbox_t m_app_exit_err   = {.obj       = &pdata->msgbox_app_exit_err,
                                        .title     = "Error",
    ...
    struct msgbox_t m_app_exit_many_times = {.obj       = &pdata->msgbox_app_exit_many_times,
                                             .title     = "Error",
                                             .text      = many_txt,
                                             .btn_close = true,
                                             .btn_ids   = NULL,
    ...
    struct msgbox_t m_export_logs_err         = {.obj       = &pdata->msgbox_export_logs_err,
                                                 ...
                                                 .btn_ids   = NULL,
                                                 .btn_texts = NULL};
    static int      msgbox_update_btn_ids[]   = {BTN_UPDATE_FOUND_OK, BTN_UPDATE_FOUND_CANCEL};
    static char    *msgbox_update_btn_texts[] = {"UPDATE", "CANCEL", "\0"};
    struct msgbox_t m_update_found            = {.obj       = &pdata->msgbox_update_found,
                                                 .title     = "Update found",
    ...
    struct msgbox_t msgboxes[] = {m_sup_ver,         m_app_ver,      m_app_start_already,   m_app_start_succ,
                                  m_app_start_err,   m_app_exit_err, m_app_exit_many_times, m_export_logs_succ,
                                  m_export_logs_err, m_update_found, m_update_already,      m_update_succ,
                                  m_update_err};
