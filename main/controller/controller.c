        ...
        switch (msg) {
            case LV_PMAN_CONTROLLER_MSG_TAG_EXPORT_LOGS: {
                ...
                for (int i = 0; i < model_get_n_log_paths(pmodel); i++) {
                    ...
                    char *fullpath = malloc(strlen(model_get_mount_path(pmodel)) + strlen(name) + 2);
            ...
            case LV_PMAN_CONTROLLER_MSG_TAG_UPDATE_COPY: {
                log_info("copio");
                if (copy_file(model_get_update_path(pmodel), model_get_app_path(pmodel))) {
                    ...
                    get_app_version(pmodel, model_get_app_path(pmodel), model_get_app_version(pmodel));
...
void controller_fork_start(model_t *pmodel, const char *path) {
    ...
    if ((pid = fork()) == -1) {
        ...
        if (WIFEXITED(status)) {
            ...
            if (es == 0) {
                if (get_millis() - start < model_get_period(pmodel) && ++term >= model_get_term_per_period(pmodel)) {
